...
import { FcBusinessman, FcCancel, FcGraduationCap, FcOk } from "react-icons/fc";
...
const AdminHome = () => {
  ...
  const handleOpen = (items,index) => {
    setOpen(true);
    setData(items);
    setApplicationId(index+1)
  };
  const handleClose = () => {
    setOpen(false);
    setData(null);
  ...
  const updateApplication =async(status)=>{
    ...
    alert("Updated succesfully")
      ...
      alert("Error")
      console.log("cannot update applications", error);
    }
      ...
      <div className="bg-[#2d3250] w-full h-screen fixed">
        <div className="w-full h-24 bg-[#31354e] border-b-2 p-6 border-b-[#6b6d77b4] ">
          <div className="flex justify-start text-[#e6ac00] ms-40">
            ...
            <div className="text-slate-50 ms-5 text-3xl font-mono mt-1">
              Scholar Hub
            </div>
          </div>
          <div className="flex justify-end m-[-37px] ms-40">
            <div className="pe-3">
              <Avatar>A</Avatar>
            </div>
            <div className="pe-10 text-lg font-semibold text-slate-200 pt-1 me-5">
              Admin Name
            </div>
          </div>
        </div>
                  ...
                  <div>
                    <div className="flex flex-col">
                      <div
                        ...
                        onClick={() =>
                          handleColumnClick("Content for Add Scholarship")
                        }
                      >
                        Add Scholarship
                      </div>
                ...
                <Item className="h-screen overflow-auto">
                  <div>
                    {selectedContent === "Content for Add Scholarship" ? (
                      <section className="bg-white dark:bg-gray-900 ">
                        <div className="py-8 px-4 mx-auto max-w-2xl lg:py-16 ">
                          ...
                          <form onSubmit={handleSubmit}>
                            ...
                            <button
                              type="submit"
                              className="inline-flex items-center bg-slate-600 px-5 py-2.5 mt-4 mb-20 sm:mt-6 text-sm font-semibold text-center text-white bg-white-200 rounded-lg  focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-slate-100 hover:text-slate-800 "
                            >
                              Add Scholarship
                            </button>
                          </form>
                      ...
                      <div>
                        <section
                          className="w-[100%] h-[85vh] bg-gray-800 
                              ...
                              <div key={index}>
                                <div className="border-2 rounded-lg bg-white w-[600px] h-[200px] mb-6 p-4">
                                  <div className="flex mb-5 border-b-2 py-2">
                                    ...
                                    />
                                    <h1 className="text-black mt-1 ml-2 font-semibold text-[18px]">
                                      {" "}
                                      {item.name}
                                    </h1>
                                  </div>
                                  ...
                                  <div className="flex mb-6">
                                    ...
                                    />
                                    <h1 className="text-black mt-0 ml-2 font-semibold text-[18px]">
                                      {" "}
                                      {item.scholarship}
                                    </h1>
                                  </div>
                                  ...
                                  <button
                                    className="font-semibold mt-5 text-blue-700"
          ...
          <Fade in={open}>
            <Box sx={style}>
              <div style={{ fontFamily: "sans-serif", color: "white" }}>
                <h1 className="text-[20px] font-semibold items-center text-center border-b-2 border-b-white w-[600px] mx-auto mb-12 p-2">
                  Scholarship Application Details
                </h1>
                <div className="grid grid-cols-2 ml-[100px]">
                  <div className="col-span-1">
                    <h1 className="text-center text-[19px] font-semibold mb-3">
                      Name :{" "}
                    </h1>
                    <h1 className="text-center text-[19px] font-semibold mb-3">
                      Scholarship :{" "}
                ...
                {/* {data.certificate && (
                    <div>
                      <Typography variant="subtitle1" component="h3">Certificate:</Typography>
                      {renderFile(data.certificate)}
                    </div>
                  )}
                  {data.identity && (
                    <div>
                      <Typography variant="subtitle1" component="h3">Identity Proof:</Typography>
                      {renderFile(data.identity)}
                    </div>
                  )}
                  {data.photo && (
                    <div>
                      <Typography variant="subtitle1" component="h3">Photograph:</Typography>
                      {renderFile(data.photo)}
                    </div>
                  )} */}

                <div className="flex justify-evenly mt-[70px] w-[400px] mx-auto ">
                  <button onClick={()=>updateApplication('accepted')} className="buttons1 flex gap-2 text-[16px] p-2 rounded-lg border-2 border-slate-600"
                   disabled={status === "accepted" || status === "rejected"} >
                    Approve <FcOk className="text-[20px]" />{" "}
                  </button>
                  <button onClick={()=>updateApplication('rejected')} className=" buttons2 flex gap-2 text-[16px]  p-2 rounded-lg border-2 border-slate-600"
                   disabled={status === "accepted" || status === "rejected"}>
                    Reject <FcCancel className="text-[20px]" />{" "}
                  </button>
                </div>
                {status && (
  <div className={`status-message ${status}`}>
           {status === "accepted" ? (
          <p className="text-green-500 text-center font-semibold mt-2">Application Approved</p>
             ) : (
           <p className="text-red-500 text-center font-semibold mt-2">Application Rejected</p>
            )}
        </div>
        )}          
              </div>
